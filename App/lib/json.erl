...
encode(L) when is_list(L) ->
	...
	no -> 
            % io:format("NO~n"), 
            exit({json_encode, {not_string, L}})
    end;
encode({array, Props}) when is_list(Props) -> 
    %% io:format("encode(Array) is list ~n"),
    encode_array(Props);
encode({struct, Props} = T) when is_list(Props) ->
    %% io:format("encode(prop) is list ~n"), 
    encode_object(T);
encode(Bad) -> %% io:format("encode(bad) is list ~n"),
